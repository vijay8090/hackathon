...
//		 List the unlocked files in datetime order, oldest first
		if (csvFiles != null && csvFiles.length > 0) {
			...
			for (File csvFile : csvFiles) {
				...
				boolean processResult = processCsvFile(new File(CsvLoaderConstants.DIR_SUBMIT_WORKING+File.separator+csvFile.getName()));
				...
				if(!processResult){
					...
					try {
						...
						//CommonUtil.sendReminderMail(null,"NVH Data Uploader failed during new channel confirmation process" , "NVH Data Uploader failed during new channel confirmation process <br/> Details : "+e.getMessage(), true);
						logger.info("CSV Data Loader failed  <br/> Details : "+e.getMessage()+ CsvLoaderConstants.DIR_SUBMIT_FAILED);
		...
		started = false;
		logger.debug("end startJobMonitor");
//		checkHoldRecords();
		return result;
	}
	...
	private File[] getCsvFiles(String dir) {
		...
		// Scan the landing zone/pending directory for unlocked files
	...
	private File[] sortFilesOldestFirst(File[] files) {
		...
		for (File f : files) {
			logger.debug("Name : " + f.getName() + " modified date : "
